  ...
  ajaxCall(
    ...
    (passengers) => {
      ...
      passengers.forEach((p, idx) => {
        ...
        tableHtml += `
         <tr data-passenger-id="${p.OtherPersonID}" data-ride-id="${p.RideID}" data-event-id="${p.EventID}" class="main-row ${rowClass} ${inactiveClass} ${confirmedClass}" data-row-idx="${idx}">
            ...
            <td data-label="שם" class="${inactiveClass} ${confirmedClass}">${p.OtherPersonName}</td>
            <td data-label="טלפון" class="${inactiveClass} ${confirmedClass}">${phoneCellHtml}</td>
            <td data-label="כתובת" class="${inactiveClass} ${confirmedClass}">${p.PickUpLocation ?? p.RideExitPoint ?? "לא צוינה"}</td>
            <td data-label="אירוע" class="${inactiveClass} ${confirmedClass}">${p.EventDesc}</td>
             <td
            <div class="extra-details-mobile" style="display:none; margin-top:8px;">
                <div><strong>סוג:</strong> ${cardTitle}</div>
                <div><strong>מס' נוסעים:</strong> ${p.NumberOfPassengers ?? 1}</div>
                <div><strong>מגדר:</strong> ${p.Gender === "M" ? "גבר" : p.Gender === "F" ? "אישה" : "לא ידוע"}</div>
                <div><strong>מעשן:</strong> ${p.Smoke ? "כן" : "לא"}</div>
                <div><strong>הערה:</strong> ${note}</div>
              </div>
            </td>
            <td class="${inactiveClass} ${confirmedClass}">
              <button class="decline-offer-btn upgraded-btn" title="ביטול הצעה" ${p.RideStatus === "Confirmed" || Number(p.IsActive) === 0 ? "disabled style='opacity:0.5;cursor:not-allowed;'" : ""}><span class="icon-x">✖️</span> ביטול</button>
              <button class="approve-offer-btn upgraded-btn" title="אישור הצעה" ${p.RideStatus === "Confirmed" || Number(p.IsActive) === 0 ? "disabled style='opacity:0.5;cursor:not-allowed;'" : ""}><span class="icon-check">✔️</span> אישור</button>
            </td>
          </tr>
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ajaxCall(
              ...
              JSON.stringify(parseInt(personID)),
